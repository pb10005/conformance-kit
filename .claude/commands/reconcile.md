...
- **要件の欠落** → AC を追加、または `then` を具体化する。変更前後を並べて提示し、**人間の承認を得てから** requirements.yaml を編集する
- **スコープの誤り** → `out_of_scope` に追記する
- **仮定の確定** → `assumptions` の該当項目を `assumed` → `confirmed` にし、コード側の `@assumption` コメントを削除する
