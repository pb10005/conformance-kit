...
AIによるコード生成が速くなるほど、ボトルネックは「書くこと」から「レビュー・検証すること」へ移る。2026年の実態調査では、85%が\
このボトルネック移行を認め、96%がAI生成コードの正しさを完全には信頼せず、AI生成PRのレビュー待ち時間は4.6倍に達すると報告されて\
いる（[出典](https://www.flowverify.co/blog/ai-code-review-bottleneck-2026-data)）。conformance-kitは\
この「検証ボトルネック」を、人間のレビュー能力ではなく機械検証・独立サブエージェントに委ねることで解こうとする。
